...
export async function handleContactSubmission(body) {
  ...
  if (!name?.trim() || !email?.trim() || !message?.trim()) {
    return {
      status: 400,
      body: { error: "Nombre, email y mensaje son obligatorios." },
    };
  }

  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!emailRegex.test(email.trim())) {
    return { status: 400, body: { error: "El email no es válido." } };
  }
  ...
  try {
    ...
    return {
      status: 500,
      body: { error: "No se pudo enviar el mensaje. Inténtalo más tarde." },
    };
  }
}
